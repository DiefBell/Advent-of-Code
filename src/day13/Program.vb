	...
	Sub PartOne(games As List(Of Game))
		...
		For Each game As Game In games
			...
			If result.Item1 <> 0 AndAlso result.Item2 <> 0 Then
				...
				Console.WriteLine($"[part 1] Target solvable for game: {game}")
			Else
				' Mark the game as unsolvable by setting the cost to -1
				game.SetCost(-1)
				Console.WriteLine($"[part 1] Target not solvable for game: {game}")
			End If
		Next
	
		' Output the total minimum cost
		Console.WriteLine($"[part 1] Total minimum cost across all games: {totalMinCost}")
	End Sub
		...
		For Each game As Game In games
			...
			If game.Cost <> -1 Then
				...
				If DPHelper.IsTargetSolvable(targetX, targetY, buttonA, buttonB) Then
					Console.WriteLine($"[part 2] Target solvable for game: {game}")
				...
				Else
					...
					Console.WriteLine($"[part 2] Target not solvable for game: {game}")
				End If
			Else
